...
const EditPoemPage: React.FC = () => {
  ...
  useEffect(() => {
    const loadData = async () => {
      refreshUser(false);
      try {
        const [occData, poemData] = await Promise.all([
          poemService.getOccasions(),
          getPoemId() ? poemService.getPoemById(getPoemId()!) : null
        ]);
        ...
        if (poemData) {
          setFormData({
            ...
            poet_name: poemData.poet_name,
            year: poemData.year || new Date().getFullYear(),
            category: poemData.category as PoemCategory,
            occasion_id: poemData.occasion_id || ''
          });
          setExistingPdfUrl(poemData.media_url || null);
        } else {
            setModal({
                isOpen: true,
                title: 'خطأ',
                message: 'لم يتم العثور على القصيدة',
                type: 'danger',
                onConfirm: () => { window.location.hash = '#/my-poems'; }
            });
        }
      } catch (e) {
        console.error(e);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setIsLoading(true);
    try {
      await poemService.updatePoem(poemId, {
    ...
    <div className="px-6 py-6 pb-24 space-y-8 page-transition">
      <div className="bg-white rounded-3xl p-8 shadow-sm">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="pt-4">
            <label className="block text-sm font-bold text-gray-700 mb-4">الملف المرفق (PDF, Word, Image) (اتركه فارغاً للاحتفاظ بالملف الحالي)</label>
            <div className="relative border-2 border-dashed border-gray-200 rounded-3xl p-8 text-center hover:border-gold transition-colors cursor-pointer group">
              ...
              <div className="flex flex-col items-center gap-2">
                ...
                <span className="text-sm font-bold text-gray-500">
                  {file ? file.name : (existingPdfUrl ? 'ملف مرفق حالياً. اختر ملفاً جديداً لاستبداله.' : 'اسحب الملف هنا أو اضغط للاختيار')}
                </span>
              </div>
            </div>
            ...
            {file && (
              <div className="mt-6 p-4 bg-gray-50 rounded-3xl border border-gray-100 animate-fadeIn">
                ...
                >
                  إزالة الملف والاحتفاظ بالملف القديم
                </button>
              </div>
            )}
          </div>
